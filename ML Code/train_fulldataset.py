...
if __name__ == "__main__":
    ...
    #open original images and convert them to tensors so they can be ran through the model
    originalImageNight = Image.open('ML Code/original_image/night.jpg').convert("RGB")
    originalImageDay = Image.open('ML Code/original_image/day.jpg').convert("RGB")
    
    originalImageNight = np.array(originalImageNight)
    originalImageDay = np.array(originalImageDay)
    
    #print(originalImageNight.shape)
    
    originalImageNight = torch.tensor(originalImageNight / 255.0, dtype=torch.float32).permute(2,0,1)
